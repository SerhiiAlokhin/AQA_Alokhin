...
count_of_results = 5 # Максимальна кількість результатів
sorted_car_by_year = sorted(car_data.items(), key=lambda x: x[1][-1])
i = 0
print("\nСортований список:")
for car_id, car_list in sorted_car_by_year:
    if car_list[1] >= search_criteria[0] and car_list[2] >= search_criteria[1] and car_list[4] <= search_criteria[2] and i < count_of_results:
        i += 1
        print(f"{i}.{car_id}, {car_list}")
